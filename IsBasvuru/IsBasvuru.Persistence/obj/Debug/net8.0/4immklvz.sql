...
CREATE TABLE `MasterGorevler` (
    `Id` int NOT NULL AUTO_INCREMENT,
    `MasterGorevAdi` longtext CHARACTER SET utf8mb4 NOT NULL,
    CONSTRAINT `PK_MasterGorevler` PRIMARY KEY (`Id`)
) CHARACTER SET=utf8mb4;
...
CREATE TABLE `Gorevler` (
    `Id` int NOT NULL AUTO_INCREMENT,
    `MasterGorevId` int NOT NULL,
    `MasterDepartmanId` int NOT NULL,
    CONSTRAINT `PK_Gorevler` PRIMARY KEY (`Id`),
    CONSTRAINT `FK_Gorevler_MasterDepartmanlar_MasterDepartmanId` FOREIGN KEY (`MasterDepartmanId`) REFERENCES `MasterDepartmanlar` (`Id`) ON DELETE CASCADE,
    CONSTRAINT `FK_Gorevler_MasterGorevler_MasterGorevId` FOREIGN KEY (`MasterGorevId`) REFERENCES `MasterGorevler` (`Id`) ON DELETE CASCADE
) CHARACTER SET=utf8mb4;
...
CREATE TABLE `GorevAtamaDetaylari` (
    `Id` int NOT NULL AUTO_INCREMENT,
    `PersonelId` int NOT NULL,
    `MasterDepartmanId` int NOT NULL,
    `GorevId` int NOT NULL,
    `PanelKullaniciId` int NOT NULL,
    `NetUcret` int NOT NULL,
    `TalepEdilenGorevGenelButcesi` int NOT NULL,
    `BaslangicTarihi` date NOT NULL,
    `TalepNedeni` int NOT NULL,
    `YerineAlinacakKisiAdSoyad` longtext CHARACTER SET utf8mb4 NULL,
    CONSTRAINT `PK_GorevAtamaDetaylari` PRIMARY KEY (`Id`),
    CONSTRAINT `FK_GorevAtamaDetaylari_Gorevler_GorevId` FOREIGN KEY (`GorevId`) REFERENCES `Gorevler` (`Id`) ON DELETE RESTRICT,
    CONSTRAINT `FK_GorevAtamaDetaylari_MasterDepartmanlar_MasterDepartmanId` FOREIGN KEY (`MasterDepartmanId`) REFERENCES `MasterDepartmanlar` (`Id`) ON DELETE RESTRICT,
    CONSTRAINT `FK_GorevAtamaDetaylari_PanelKullanicilari_PanelKullaniciId` FOREIGN KEY (`PanelKullaniciId`) REFERENCES `PanelKullanicilari` (`Id`) ON DELETE RESTRICT,
    CONSTRAINT `FK_GorevAtamaDetaylari_Personeller_PersonelId` FOREIGN KEY (`PersonelId`) REFERENCES `Personeller` (`Id`) ON DELETE RESTRICT
) CHARACTER SET=utf8mb4;
...
CREATE INDEX `IX_GorevAtamaDetaylari_GorevId` ON `GorevAtamaDetaylari` (`GorevId`);

CREATE INDEX `IX_GorevAtamaDetaylari_MasterDepartmanId` ON `GorevAtamaDetaylari` (`MasterDepartmanId`);

CREATE INDEX `IX_GorevAtamaDetaylari_PanelKullaniciId` ON `GorevAtamaDetaylari` (`PanelKullaniciId`);

CREATE INDEX `IX_GorevAtamaDetaylari_PersonelId` ON `GorevAtamaDetaylari` (`PersonelId`);

CREATE INDEX `IX_Gorevler_MasterDepartmanId` ON `Gorevler` (`MasterDepartmanId`);

CREATE INDEX `IX_Gorevler_MasterGorevId` ON `Gorevler` (`MasterGorevId`);
...
VALUES ('20260309101244_Initalcreate', '8.0.2');
